...
A fresh install comes with eight pieces — two sarees, three kurtis, a dress, a
lehenga and a blouse — so the kiosk can be demonstrated and staff trained before
any real stock is photographed.

**Their pictures are drawings, not photographs.** They are generated by
`npm run seed-images` into `public/seed/` and copied into the device's image
store on first run. Nothing in the seed catalogue is a real garment, and none of
it should be shown to a customer as something the shop stocks — replace each
piece with a real photo from **Staff → Catalogue** as the stock is shot.

Installs that were seeded before the illustrations existed pick them up once, on
next launch, and only for pieces the shop has not renamed or photographed
itself.
...
`android/` wraps the same `dist/` output in a native shell (`com.saaj.tryon`),
for when the tablet should run an installed app rather than an installed PWA.
...
`npx cap sync` is what copies `dist/` into `android/app/src/main/assets/public`,
so it has to run after every `npm run build` — that copied output is generated,
not source, and is gitignored.

The customer photo is captured with `getUserMedia` **inside the WebView**, not
through a native camera plugin. Capacitor's WebChromeClient only grants that
request if the app itself holds `CAMERA`, so the manifest declares it; both
camera `uses-feature` entries are `required="false"` so a camera-less tablet
still installs and falls back to picking a file. If you ever strip the manifest
back, the camera silently stops working in the packaged app while the PWA keeps
working — that is the failure mode to watch for.
